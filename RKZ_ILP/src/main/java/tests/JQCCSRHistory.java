	...
	public static void history(String SSN,String AppURL) throws ParseException, InterruptedException
	{
	 ...
	 //ACH collateral details from sheet
			 ...
			 //Validations
						...
						String loan_account_inf_inst_amt= driver.findElement(locator(Jprop.getProperty("loan_account_inf_loan_date"))).getText();
						String loan_account_inf_no_of_installments= driver.findElement(locator(Jprop.getProperty("loan_account_inf_max_term_date"))).getText();
						...
						String cust_promotion_coupon_code= driver.findElement(locator(Jprop.getProperty("cust_promotion_coupon_code"))).getText();
...
//Loan paid to date summary
						...
						else
						{
							...
							if(courtesy_consent.toLowerCase().trim().equalsIgnoreCase(courtesy_inf_courtesy_consent.toLowerCase().trim()))
							{
					...
					if(total_paid_amountc.equalsIgnoreCase(loan_paid_summary_total_paid_amt))
							...
							if(cust_doc_tender_type_test.trim().equalsIgnoreCase(cust_doc_tender_type.trim())||" ".trim().equalsIgnoreCase(cust_doc_tender_type.trim()))
							{
